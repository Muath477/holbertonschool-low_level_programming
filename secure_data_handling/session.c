...
session_t *session_create(const char *id, unsigned int uid, const unsigned char *data, size_t data_len)
{
	...
	/* own our own copy of id, don't just alias the caller's buffer */
	if (id) {
		id_len = strlen(id);
...
int session_set_data(session_t *s, const unsigned char *data, size_t data_len)
{
	...
	/* don't overwrite s->data until we know realloc actually worked,
	 * otherwise we lose the old pointer and leak it on failure */
